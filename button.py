...
class Button:
    ...
    def __init__(self, ax: int, ay: int, bx: int, by: int, text: str, function: FunctionType) -> None:
        ...
        self.button = fl.rectangle(
            ax, ay, bx, by, couleur='grey', remplissage='grey', epaisseur=1, tag='')
        self.btn_text = fl.texte(ax+5, ay+5, text, couleur='black',
    ...
    def update(self, event: Event) -> None:
        ...
        """
        textx, texty = fl.taille_texte(self.text, taille=20)
        if self.is_hovered():
            self.button = fl.rectangle(
                self.ax, self.ay, self.bx, self.by, couleur='grey', remplissage='red', epaisseur=1, tag='')
            self.btn_text = fl.texte(self.bx - self.ax - textx/2, self.by - (self.by - self.ay)/2 - texty/2, self.text, couleur='black',
                                     ancrage='nw', police='Helvetica', taille=20, tag='')
